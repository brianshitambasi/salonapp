              ...
              <Card className="h-100 shadow-sm">
                {post.imageUrl && (
                  <div style={{ position: 'relative', cursor: 'pointer', backgroundColor: '#000', minHeight: '250px' }} onClick={() => openMediaModal(post.imageUrl, isVideo ? 'video' : 'image')}>
                    {isVideo ? (
                      <>
                        <video src={post.imageUrl} style={{ height: '250px', width: '100%', objectFit: 'cover' }} />
                        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'rgba(0,0,0,0.7)', borderRadius: '50%', padding: '15px', color: 'white' }}>
                          <FaPlay size={30} />
                        </div>
                      </>
                    ) : (
                      <img src={post.imageUrl} alt={post.title} style={{ height: '250px', width: '100%', objectFit: 'cover' }} />
                    )}
                  </div>
                )}
                <Card.Body>
                  ...
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <button className="btn btn-link text-primary p-0" onClick={() => { setSelectedPost(post); setShowCommentModal(true); }}>
                      <FaComment /> <span className="ms-1">{post.comments?.length || 0} comments</span>
                    </button>
                  </div>
                  {isAdmin && (
                    <div className="d-flex justify-content-end gap-2 mt-2">
                      <Button size="sm" variant="warning" onClick={() => { setEditingPost(post); setFormData({ title: post.title, imageUrl: post.imageUrl, description: post.description }); setPreviewUrl(post.imageUrl); setSelectedFile(null); setShowPostModal(true); }}><FaEdit /> Edit</Button>
                      <Button size="sm" variant="danger" onClick={() => deletePost(post._id)}><FaTrash /> Delete</Button>
                    </div>
                  )}
                  {post.comments?.slice(0, 3).map(comment => (
                    <div key={comment._id} className="mt-2 p-2 bg-light rounded small">
                      <div className="d-flex justify-content-between">
                        ...
                        {(isAdmin || comment.userId?._id === user?._id) && <Button size="sm" variant="link" className="text-danger p-0" onClick={() => deleteComment(post._id, comment._id)}><FaTrash size={10} /></Button>}
                      </div>
