...
const AdminDashboard = () => {
  ...
  const [staffForm, setStaffForm] = useState({ 
    name: '', role: '', bio: '', imageUrl: '', experience: '', 
  ...
  const handleStaffSubmit = async (e) => {
    ...
    let imageUrl = staffForm.imageUrl;
    if (staffSelectedFile) {
      const uploadedUrl = await uploadFile(staffSelectedFile);
      if (uploadedUrl) imageUrl = uploadedUrl;
    }
    
    const staffData = { ...staffForm, imageUrl };
    
    try {
      if (editing) {
        await staffAPI.update(editing._id, staffData);
              ...
              <Card.Body>
                <Form onSubmit={handleServiceSubmit}>
                  <Row>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder="Service name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} required />
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                  ...
                  <Row>
                    ...
                    <Col md={6}><Form.Group><Form.Label>Benefits</Form.Label><div className="d-flex"><Form.Control value={benefitInput} onChange={e => setBenefitInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), addBenefit())} /><Button type="button" onClick={addBenefit} className="ms-2">Add</Button></div><div className="mt-2">{form.benefits.map(benefit => <Badge key={benefit} bg="info" className="me-1 mb-1" style={{ cursor: 'pointer' }} onClick={() => removeBenefit(benefit)}>✓ {benefit} ✕</Badge>)}</div></Form.Group></Col>
                  </Row>
                  ...
                  <Row>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder="Name" value={staffForm.name} onChange={e => setStaffForm({...staffForm, name: e.target.value})} required />
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        <Form.Label>Role / Title</Form.Label>
                        <Form.Control placeholder="e.g., Senior Stylist" value={staffForm.role} onChange={e => setStaffForm({...staffForm, role: e.target.value})} required />
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                  ...
                  <Row>
                    <Col md={12}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Control as="textarea" rows={2} placeholder="About the stylist..." value={staffForm.bio} onChange={e => setStaffForm({...staffForm, bio: e.target.value})} />
                      </Form.Group>
                    </Col>
                  </Row>
                  <Row>
                    <Col md={3}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Control type="number" placeholder="Years" value={staffForm.experience} onChange={e => setStaffForm({...staffForm, experience: e.target.value})} />
                      </Form.Group>
                    </Col>
                    <Col md={3}>
                      <Form.Group className="mb-3">
                        <Form.Label>Start Time</Form.Label>
                        <Form.Control type="time" value={staffForm.workingHours.start} onChange={e => setStaffForm({...staffForm, workingHours: {...staffForm.workingHours, start: e.target.value}})} required />
                      </Form.Group>
                    </Col>
                    <Col md={3}>
                      <Form.Group className="mb-3">
                        <Form.Label>End Time</Form.Label>
                        <Form.Control type="time" value={staffForm.workingHours.end} onChange={e => setStaffForm({...staffForm, workingHours: {...staffForm.workingHours, end: e.target.value}})} required />
                      </Form.Group>
                    </Col>
